...
from mirage.simulator.isaaclab.utils.scene import SceneCfg
from mirage.simulator.base_simulator.simulator import Simulator
from mirage.simulator.base_simulator.config import (
    MarkerState,
    ControlType,
    VisualizationMarker,
    SimBodyOrdering,
    ...
    def render(self) -> None:
        ...
        if not self.headless:
            if not hasattr(self, "_perspective_view"):
                from mirage.simulator.isaaclab.utils.perspective_viewer import (
                    PerspectiveViewer,
                )
